...
\begin{longtable}{lp{60mm}}
    ...
    $H_{rpid}= K_{rk}\Big[ \frac{(1+sT_{nk})(1+sT_{vk})}{sT_{nk}}\Big]              $ & \"Ubertragungsfunktion des PID-Reglers (bodekonform) \\
    $H_{rpi} = K_{rk}\Big[ 1 + \frac{1}{sT_{nk}} \Big]                              $ & \"Ubertragungsfunktion des PI-Reglers (bodekonform) \\
...
Zuerst wird  im Phasengang  der Strecke  die Frequenz  $\omega_{pi}$ bestimmt,
f\"ur  welche die  Phase der  Strecke $-90\degree$  betr\"agt, ersichtlich  in
...
Damit kann nun $T_{nk}$ direkt bestimmt werden\footnotemark[5]:
...
Die   Durchtrittsfrequenz  ist   die  Frequenz,   bei  der   eine  betrachtete
\"Ubertragungsfunktion   eine  Verst\"arkung   von   $\SI{0}{\decibel}  =   1$
aufweist. In  der  Phasengangmethode  soll  sie  so  festgelegt  werden,  dass
...
$\omega_d$    gefunden   werden,    an   welcher    der   offene    Regelkreis
eine   Phase  von   $-128.5\degree$  aufweist   (Gleichung~\ref{eq:phi_o}). In
Abbildung~\ref{fig:pi:omega_d} kann dies grafisch verifiziert werden.
