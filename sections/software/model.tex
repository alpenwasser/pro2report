...
\subsubsection*{Model}
Wie   im   Klassendiagramm   ersichtlich  \todo{Verweis}erzeugt   die   Klasse
\code{Model} im Konstruktor f\"ur jede  Berechnungsart eine Instanz der Klasse
\code{ClosedLoop}. Das \code{Model} hat ein  Objekt der Klasse \code{Path} und
\"ubergibt dieses an den \code{ClosedLoop}. Das \code{Model} enth\"alt Setter-
und Getter-Methoden  zur Verarbeitung der Daten  mittels weiterer Klassen. Das
\code{Model} nutzt  die M\"oglichkeit \code{notifyObservers()}  aufzurufen, um
\code{update()} auf der \code{View} zu bewirken.
...
die    Schrittantwort    mittels   \code{calculateStepResponse()}.F\"ur    die
Phasengangmethode   wird  zus\"atzlich   das  \"Uberschwingverhalten   mittels
\code{overShootOptimazation()} optimiert (siehe Anhang~\ref{app:algo:oversh}).
...
Phasengangmethode. Er beinhaltet die abstrakte Klasse \code{calculate()} sowie
alle n\"otigen Setter- und Getter-Methoden um Werte zu setzten und auszulesen.


\subsubsection*{Chien20}
Die  Klasse   \code{Chien20}  stellt   die  Algorithmen  zur   Berechnung  der
Reglerwerte   gem\"ass  der   Faustformel   Chien/Hrones/Reswick  (20\%)   zur
Verf\"ugung,  erbt   von  \code{Controller},   instanziiert  und   setzt  eine
\"Ubertragungsfunktion.


\subsubsection*{ChienApper}
Die  Klasse  \code{ChienApper}  stellt  die  Algorithmen  zur  Berechnung  der
Reglerwerte  gem\"ass   der  Faustformel   Chien/Hrones/Reswick  (aperiodisch)
zur  Verf\"ugung,  erbt von  \code{Controller},  instanziiert  und setzt  eine
\"Ubertragungsfunktion.


\subsubsection*{Oppelt}
Die Klasse \code{Oppelt} stellt die Algorithmen zur Berechnung der Reglerwerte
gem\"ass der  Faustformel Oppelt zur Verf\"ugung,  erbt von \code{Controller},
instanziiert und setzt eine \"Ubertragungsfunktion.


\subsubsection*{Rosenberg}
Die  Klasse  \code{Rosenberg}  stellt   die  Algorithmen  zur  Berechnung  der
